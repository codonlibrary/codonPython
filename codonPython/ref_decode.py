...
''''Main Function'''
def ref_decode(df, org_cols=[], sno_cols=[], icd10_cols=[]):
    ...
    df_decoded = df.copy()
    if org_cols != []:
        df_decoded = org_col(df_decoded,org_cols)
    if sno_cols != []:
        df_decoded = sno_col(df_decoded,sno_cols)
    if icd10_cols != []:
        df_decoded = icd10_col(df_decoded,icd10_cols)
    return df_decoded

'''Sub functions'''
def org_col(df, column_list, org_lkp=None):
    ...
    if org_lkp is None:
        df_org_lkp = dss_org_api().set_index('OrganisationId')['Name']  # this allows us to pull names easily
    # join the columns on
    df_out = df.copy()
...
def sno_col(df, column_list,df_sno_lkp=None):
    ...
    if df_sno_lkp is None:
        df_sno_lkp = SNO_lkp().set_index('CONCEPT_ID')
    df_out = df.copy()
    for column in column_list:
...
def icd10_col(df, column_list, fourchar=False, df_icd10_lkp=None):
    ...
    if df_icd10_lkp is None:
        df_icd10_lkp = cc.ICD10_lkp()[[code, 'ICD10_DESCRIPTION']].set_index(code)
    if type(column_list) != type([]):
        column_list = [column_list]
    df_out = df.copy()
    for column in column_list:
...
def ICD10_lkp():
    ...
    df_ICD10_lkp = pd.read_sql_query(ICD10_sql, conn_DSS2016UAT())
    return df_ICD10_lkp
...
def SNO_lkp():
    ...
    SNO_sql = '''
                ...
                ORDER BY CONCEPT_ID
                '''
    df_SNO = pd.read_sql_query(SNO_sql,conn_DSS2016UAT())
    return df_SNO
